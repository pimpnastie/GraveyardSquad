...
class ClashRoyale(commands.Cog):
    def __init__(self, bot):
        ...
        try:
            ...
            if loop.is_running(): asyncio.ensure_future(self._cache_cards())
            else: loop.create_task(self._cache_cards())
    ...
    async def cog_before_invoke(self, ctx):
        if ctx.command and ctx.command.name in WARMUP_RELEVANT_COMMANDS: await self._check_and_start_warmup()
        if not self.all_cards and ctx.command and ctx.command.name in {"whohas"}: await self._cache_cards()

    async def cog_command_error(self, ctx, error):
        if isinstance(error, commands.CommandOnCooldown): await ctx.send(f"⏳ **Woah there!** Please wait {error.retry_after:.1f} seconds.")
        else: log.error(f"Error in command '{ctx.command}': {error}")

    async def _check_and_start_warmup(self):
        warm_key = f"warmed_today:{self.clan_tag}"
        is_warmed = await self._cache_get(warm_key)
        if is_warmed or self.clan_tag in self.active_warmups: return
    ...
    async def _run_warmup_task(self):
        ...
        except Exception as exc: log.error(f"Error during warmup for #{self.clan_tag}: {exc}")
        finally: self.active_warmups.discard(self.clan_tag)
    ...
    async def wait_if_warming(self, ctx):
        if self.clan_tag in self.active_warmups:
            ...
            while self.clan_tag in self.active_warmups: await asyncio.sleep(1)
            await msg.delete()
    ...
    async def _cache_set(self, key: str, value, ttl: int):
        if self.bot.redis_available: await self.bot.redis.setex(key, ttl, json.dumps(value))
        else: await self.mongo_cache.update_one({"_id": key}, {"$set": {"data": value, "expires_at": time.time() + ttl}}, upsert=True)

    async def _cache_delete(self, key: str):
        if self.bot.redis_available: await self.bot.redis.delete(key)
        else: await self.mongo_cache.delete_one({"_id": key})
    ...
    def _is_maxed(self, card: dict) -> bool:
        ...
        if max_level is not None: return level >= max_level
        rarity = card.get("rarity", "").lower()
        rarity_max = {"common": 14, "rare": 12, "epic": 10, "legendary": 9, "champion": 9}
        fallback = rarity_max.get(rarity)
        return level >= fallback if fallback is not None else False
    ...
    async def run_harvest_logic(self):
        ...
        if war_data:
            if "clan" in war_data and war_data["clan"] and "participants" in war_data["clan"]:
                war_participants = {p["tag"].replace("#", "").upper(): p for p in war_data["clan"]["participants"]}
            else:
                for clan in war_data.get("clans", []):
                    if clan and clan.get("tag", "").replace("#", "").upper() == self.clan_tag.upper():
                        war_participants = {p["tag"].replace("#", "").upper(): p for p in clan.get("participants", [])}
                        break
        ...
        for tag, m, profile, blog in results:
            flat_data = {
                "date": snapshot_date, "name": m.get("name", ""), "tag": tag, "trophies": m.get("trophies", 0),
                "role": m.get("role", "member"), "donations": m.get("donations", 0), "donationsReceived": m.get("donationsReceived", 0),
            }
            if tag in war_participants:
                wp = war_participants[tag]
                flat_data.update({"fame": wp.get("fame", 0), "decksUsedToday": wp.get("decksUsedToday", 0), "decksRemaining": wp.get("decksRemaining", 4)})
            ...
            if blog and isinstance(blog, list):
                for battle in blog:
                    ...
                    crowns_team, crowns_opp = team_data.get("crowns", 0), opp_data.get("crowns", 0)
                    result_str = "win" if crowns_team > crowns_opp else "loss" if crowns_opp > crowns_team else "draw"
                    battle_doc = {
                        "player_tag": tag, "battle_time": battle["battleTime"], "type": battle.get("type", "unknown"),
                        "result": result_str, "team_crowns": crowns_team, "opp_crowns": crowns_opp,
                        "opp_name": opp_data.get("name", "Unknown"), "opp_tag": opp_data.get("tag", ""),
                    }
                    battle_ops.append(UpdateOne({"_id": battle_id}, {"$set": battle_doc}, upsert=True))
        snap_count = prof_count = battle_count = 0
        if snapshot_ops: await self.db["historical_snapshots"].bulk_write(snapshot_ops, ordered=False); snap_count = len(snapshot_ops)
        if profile_ops: await self.db["player_profiles"].bulk_write(profile_ops, ordered=False); prof_count = len(profile_ops)
        if battle_ops: await self.db["battle_history"].bulk_write(battle_ops, ordered=False); battle_count = len(battle_ops)
        mainbot._harvest_meta.update({
            "status": "ok", "last_run": datetime.now().isoformat(), "snapshots_saved": snap_count, "profiles_saved": prof_count,
            "battles_saved": battle_count, "duration_s": round(time.monotonic() - harvest_start, 1),
            "member_count": len(members), "war_participants_found": len(war_participants),
        })
        await self.db["config"].update_one(
            {"_id": "harvest_meta"}, {"$set": {**mainbot._harvest_meta, "last_run": datetime.now().isoformat()}}, upsert=True,
        )
        log.info(f"✅ Harvest complete — {snap_count} snapshots, {prof_count} profiles, {battle_count} battles in {mainbot._harvest_meta['duration_s']}s")
    ...
    async def daily_snapshot_loop(self): await self.run_harvest_logic()

    @daily_snapshot_loop.before_loop
    async def before_daily_snapshot_loop(self): await self.bot.wait_until_ready()
        
    @daily_snapshot_loop.error
    async def on_snapshot_error(self, error):
    ...
    async def p(self, ctx, *, target: str = None):
        ...
        if target:
            if target.startswith("<@"):
                ...
                if all(c in set("0289CGJLPQRUVY") for c in clean_target) and len(clean_target) > 3: tag_to_search = clean_target
                else:
                    ...
                    if clan_data and "memberList" in clan_data:
                        ...
                        if score >= 70:
                            for m in members:
                                if m["name"] == match: tag_to_search = m["tag"].replace("#", ""); break
                    if not tag_to_search: return await ctx.send(f"❌ Could not find a valid tag or clan member matching **'{target}'**.")
        else:
    ...
    async def link(self, ctx, tag: str):
        ...
        if role:
            ...
            except discord.Forbidden: await ctx.send(f"✅ Linked **{data['name']}**, but I lack role permissions.")
        else: await ctx.send(f"✅ Linked **{data['name']}**.")
    ...
    async def chests(self, ctx, target: discord.Member = None):
        ...
        for chest in data["items"]:
            index, name = chest.get("index"), chest.get("name", "Unknown Chest")
            if index == 0: embed.add_field(name="Next Chest", value=name, inline=False)
            elif "Silver" not in name and "Gold" not in name: embed.add_field(name=f"In {index} wins", value=f"✨ {name}", inline=True)
        await ctx.send(embed=embed)
    ...
    async def log(self, ctx, target: discord.Member = None):
        ...
        for battle in data[:5]:
            ...
            crowns_team, crowns_opp = team.get("crowns", 0), opponent.get("crowns", 0)
    ...
    async def war(self, ctx):
        ...
        embed.add_field(name="Status", value=data.get("state", "Unknown").title(), inline=False)
        embed.add_field(name="Fame", value=f"⭐ {data.get('clan', {}).get('fame', 0)}", inline=True)
        await ctx.send(embed=embed)
    ...
    async def whohas(self, ctx, *, card_name: str = None):
        ...
        if not valid_profiles: return await ctx.send("❌ No member profiles could be loaded.")
        if card_name:
            ...
            counts = Counter(card["name"] for p in valid_profiles for card in p.get("cards", []) if self._is_maxed(card))
            if not counts: return await ctx.send("❌ No maxed cards found across any profiles.")
            target_card = counts.most_common(1)[0][0]
        name_by_tag = {m["tag"].replace("#", "").upper(): m["name"] for m in members}
        hits = [f"• **{name_by_tag.get(p.get('tag', '').replace('#', '').upper(), p.get('name'))}**" for p in valid_profiles for card in p.get("cards", []) if card["name"] == target_card and self._is_maxed(card)]
        response = f"📊 **Owners of {target_card} (maxed):**\n" + "\n".join(hits) if hits else f"❌ Nobody has **{target_card}** maxed."
        await ctx.send(response[:1900] + ("\n… and more." if len(response) > 1900 else ""))
    ...
    async def cardstats(self, ctx):
        ...
        clan_data = await self._get_clan_data(self.clan_tag)
        if not clan_data: return await msg.edit(content="❌ Could not fetch clan data.")
        member_list = clan_data.get("memberList", [])
        profiles = await self._fetch_members_concurrent(member_list)
        valid_profiles = [p for p in profiles if p and "cards" in p]
        if not valid_profiles: return await msg.edit(content="❌ Fetched profiles, but no card data was found.")
        card_to_members = {}
        for p in valid_profiles:
            for card in p.get("cards", []):
                if self._is_maxed(card): card_to_members.setdefault(card["name"], []).append(p.get("name", "Unknown"))
        if not card_to_members: return await msg.edit(content="❌ No maxed cards found.")
        output = io.StringIO()
        ...
        output.seek(0)
        await msg.delete()
        await ctx.send(content=f"✅ Card stats compiled!", file=discord.File(fp=output, filename="Card_Report.csv"))

    @commands.command()
    async def forecast(self, ctx):
        cached = await self._cache_get(f"currentrace:{self.clan_tag}")
        data = cached if cached else await self.bot.async_fetch_cr_api(f"clans/%23{self.clan_tag}/currentriverrace")
        ...
        if not participants and "clans" in data:
            for c in data["clans"]:
                if c.get("tag", "").replace("#", "").upper() == self.clan_tag.upper():
                    participants = c.get("participants", []); break
        if not participants: return await ctx.send("❌ No activity participants verified.")
        ...
        if projected >= 10_000: await ctx.send(f"📈 On pace! Projected: **{projected:,}** Fame.")
        else: await ctx.send(f"📉 Under pace. Projected: **{projected:,}** Fame.")
    ...
    async def primetime(self, ctx):
        ...
        await msg.delete()
        await ctx.send(f"🔥 Prime active window evaluates to **{top_hour:02d}:00 Eastern Time Zone**.")
...
async def setup(bot): await bot.add_cog(ClashRoyale(bot))
