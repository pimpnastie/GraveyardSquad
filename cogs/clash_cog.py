...
# --- Cache TTLs (seconds) ---
TTL_CARDS       = 60 * 60 * 24   # 24 hours  – card list rarely changes
TTL_CLAN        = 60 * 10        # 10 minutes
TTL_PLAYER      = 60 * 5         # 5 minutes
TTL_CLAN_TAG    = 60 * 60        # 1 hour
TTL_BATTLE_LOG  = 60 * 60 * 24   # 24 hours  – "first use of the day" log cache
TTL_WAR         = 60 * 5         # 5 minutes  – live war data
TTL_PROFILES    = 60 * 60 * 24   # 24 hours  – bulk profile warmup

# CR API retry settings
MAX_RETRIES     = 3
RETRY_BACKOFF   = 1.5            # seconds, doubles each attempt

# Commands that benefit from cache warming.
WARMUP_RELEVANT_COMMANDS = {"scout", "primetime", "cardstats", "whohas"}

# Heavy commands that should be rate-limited per guild.
HEAVY_COMMANDS_COOLDOWN = 30     # seconds
...
class ProfileView(discord.ui.View):
    ...
    def build_overview_embed(self) -> discord.Embed:
        ...
        e.set_thumbnail(
            url="https://royaleapi.github.io/cr-api-assets/arenas/arena_gold.png"
        )
        e.add_field(
            name="🏆 Trophies",
        ...
        e.add_field(
            name="👑 Crowns",
            value=(
                f"3-Crown Wins: **{self.data.get('threeCrownWins', 0)}**\n"
                f"Total Battles: **{self.data.get('battleCount', 0)}**"
            ),
            inline=False,
        )
        return e
    ...
    def build_social_embed(self) -> discord.Embed:
        e = discord.Embed(
            title=f"🛡️ Social & War | {self.data.get('name')}", color=0xE67E22
        )
        ...
        if clan:
            e.add_field(
                name="Clan",
                value=(
                    f"**{clan.get('name')}** ({clan.get('tag')})\n"
                    f"Role: {self.data.get('role', 'Member').capitalize()}"
                ),
                inline=False,
            )
        else:
        ...
        e.add_field(
            name="🎁 Donations",
            value=(
                f"Given: **{self.data.get('donations', 0)}**\n"
                f"Received: **{self.data.get('donationsReceived', 0)}**\n"
                f"Lifetime: **{self.data.get('totalDonations', 0)}**"
            ),
            inline=True,
        )
        e.add_field(
    ...
    def build_deck_embed(self) -> discord.Embed:
        e = discord.Embed(
            title=f"🃏 Deck & Collection | {self.data.get('name')}", color=0x9B59B6
        )
        ...
        deck_str = "\n".join(
            f"• ** (Lvl {card['level']})"
            for card in self.data.get("currentDeck", [])
        )
        e.add_field(
...
class ClashRoyale(commands.Cog):
        ...
        try:
            ...
            async def warm_member(member):
                async with sem:
                    ...
                    await self._api_get(
                        url_log,
                        cache_key=f"battlelog:{raw_tag}",
                        ttl=TTL_BATTLE_LOG,
                    )
                    url_player = f"{self.api_base}/players/{quote('#' + raw_tag)}"
                    await self._api_get(
                        url_player,
                        cache_key=f"player:{raw_tag}",
                        ttl=TTL_PROFILES,
                    )
    ...
    async def wait_if_warming(self, ctx, clan_tag: str):
        if clan_tag in self.active_warmups:
            msg = await ctx.send(
                "⏳ **Waking up!** Gathering today's data for the clan. "
    ...
    # ------------------------------------------------------------------ #
    # API LAYER (retry + rate-limit handling)                            #
    # ------------------------------------------------------------------ #
    ...
    async def _cache_cards(self):
        ...
        data = await self._api_get(
            f"{self.api_base}/cards", cache_key=cache_key, ttl=TTL_CARDS
        )
        if data:
    ...
    async def get_clan_tag(self, user_id: int) -> str | None:
        ...
        cached = await self._cache_get(cache_key)
        if cached:
            return cached if isinstance(cached, str) else cached.get("tag")
        ...
        if data and "clan" in data:
            ...
            await self._cache_set(cache_key, tag, TTL_CLAN_TAG)
            return tag
        return None
    ...
    async def cog_command_error(self, ctx, error):
        """Catches errors specific to this cog, such as cooldowns."""
        if isinstance(error, commands.CommandOnCooldown):
            await ctx.send(
                f"⏳ **Woah there!** That's a heavy command. "
                f"Please wait {error.retry_after:.1f} seconds before running it again."
            )
        else:
        ...
        if target:
            ...
            if user_doc:
                ...
                return await ctx.send(
                    "❌ You haven't linked your account. Use `!link <tag>` first."
                )
    ...
    async def link(self, ctx, tag: str):
        ...
        if role:
            try:
                ...
                await ctx.send(
                    f"✅ Linked **{data['name']}** (#{clean_tag}) and gave you **{role.name}**!"
                )
            except discord.Forbidden:
                await ctx.send(
                    f"✅ Linked **{data['name']}** (#{clean_tag}), but I can't manage roles."
                )
        else:
    ...
    async def stats(self, ctx, target: discord.Member = None):
        ...
        embed = discord.Embed(
            title=f"📊 Stats for {data.get('name', 'Unknown')}", color=0x00FF00
        )
        ...
        if clan:
            embed.add_field(
                name="Clan",
                value=f"🛡️ {clan.get('name')} ({clan.get('tag')})",
                inline=False,
            )
    ...
    async def decks(self, ctx, target: discord.Member = None):
        ...
        embed = discord.Embed(
            title=f"⚔️ {data['name']}'s Current Deck", color=0xEE82EE
        )
        deck_str = "\n".join(
            f"• ** (Lvl {card['level']})"
    ...
    async def chests(self, ctx, target: discord.Member = None):
        ...
        embed = discord.Embed(
            title=f"🎁 Upcoming Chests for #{clean_tag}", color=0xFFD700
        )
    ...
    async def log(self, ctx, target: discord.Member = None):
        ...
        data = await self._api_get(
            url, cache_key=f"battlelog:{clean_tag}", ttl=60 * 5
        )
        if not data:
            return await ctx.send("❌ Failed to fetch battle log.")

        embed = discord.Embed(
            title=f"⚔️ Last 5 Battles for #{clean_tag}", color=0x3498DB
        )
        for battle in data[:5]:
            ...
            result = (
                "🟢 Victory" if crowns_team > crowns_opp
                else "🔴 Defeat" if crowns_opp > crowns_team
                else "⚪ Draw"
            )
    ...
    async def clan(self, ctx):
        ...
        embed.add_field(
            name="Location",
            value=f"🌍 {data.get('location', {}).get('name', 'Unknown')}",
            inline=True,
        )
        embed.add_field(
            name="Required Trophies",
            value=f"🔒 {data.get('requiredTrophies', 0)}",
            inline=True,
        )
        await ctx.send(embed=embed)
    ...
    async def war(self, ctx):
        ...
        embed.add_field(
            ...
            value=f"🃏 {clan_info.get('periodPoints', 0)}",
            inline=True,
        )
        await ctx.send(embed=embed)
    ...
    async def race(self, ctx, period: str = "current"):
        ...
        if period.lower() == "last":
            ...
            data = await self._api_get(
                url, cache_key=f"racelog:{clan_tag}", ttl=60 * 60
            )
            ...
            url = f"{self.api_base}/clans/{quote('#' + clan_tag)}/currentriverrace"
            data = await self._api_get(
                url, cache_key=f"currentrace:{clan_tag}", ttl=TTL_WAR
            )
            if not data:
    ...
    async def whohas(self, ctx, *, card_name: str = None):
        ...
        name_by_tag = {
            m["tag"].replace("#", "").upper(): m["name"] for m in members
        }
        hits = []
        for p in profiles:
    ...
    async def cardstats(self, ctx):
        ...
        sorted_cards = sorted(
            card_to_members.items(), key=lambda item: len(item[1]), reverse=True
        )
        ...
        await msg.edit(
            content=(
                f"✅ Report complete! Found **{len(card_to_members)}** different "
                f"maxed cards across {len(profiles)} members."
            ),
    ...
    async def forecast(self, ctx):
        ...
        if period_type == "training":
            return await ctx.send(
                "📋 **Forecast:** Today is a **Training Day** — there's no finish line to forecast. "
                "Save your decks for War Day!"
            )
        ...
        if decks_used_today > 0:
            avg_fame_per_deck = fame_earned_today / decks_used_today
        else:
            avg_fame_per_deck = 150
        ...
        if projected_fame >= goal:
            await ctx.send(
                f"📈 **Forecast:** Yes! You are at **{fame:,}/{goal:,}** Fame. "
                f"At {avg_label}, you project to hit **{projected_fame:,}** Fame today."
            )
        else:
            shortfall = goal - projected_fame
            await ctx.send(
                f"📉 **Forecast:** Unlikely. You are at **{fame:,}/{goal:,}** Fame. "
                f"At {avg_label}, you project to reach **{projected_fame:,}** Fame — "
                f"**{shortfall:,}** short of the finish line."
            )
    ...
    async def scout(self, ctx):
        ...
        embed = discord.Embed(
            title="🕵️ Enemy Meta Scout",
            description="Most common cards faced by your clan recently:",
            color=0xE74C3C,
        )
    ...
    async def primetime(self, ctx):
        ...
        async def fetch_battle_log(member):
            async with sem:
                raw_tag = member["tag"].replace("#", "")
                url = f"{self.api_base}/players/{quote('#' + raw_tag)}/battlelog"
                battle_log = await self._api_get(
                    url,
                    cache_key=f"battlelog:{raw_tag}",
                    ttl=TTL_BATTLE_LOG,
                )
        ...
        for name, battle_log in results:
            ...
            for battle in battle_log:
                ...
                if len(ts) >= 15:
                    try:
                        utc_dt = datetime.strptime(ts[:15], "%Y%m%dT%H%M%S").replace(
                            tzinfo=utc_tz
                        )
            ...
            for h in range(24):
                ...
                if val > 0 and max_player_hour_count > 0:
                    ...
                    cell.fill = PatternFill(
                        start_color=f"FFFF{gb:02X}{gb:02X}",
                        end_color=f"FFFF{gb:02X}{gb:02X}",
                        fill_type="solid",
                    )
        ...
        embed = discord.Embed(
            ...
            description=(
                f"Your clan's busiest time is **{display_hour}:00 {am_pm} Eastern** "
                f"({top_count} recent battles).\n\n"
        ...
        async for g in self.guilds.find({"channel_id": {"$exists": True}}):
            ...
            if not channel:
                try:
                    ...
                    log.error(f"Error fetching channel {channel_id}: {exc}")
                    continue
            try:
                await channel.send(
                    "⚔️ **War Reminder:** Use your attacks! The river race is active."
                )
            except discord.HTTPException as exc:
                log.warning(f"Failed to send reminder to channel {channel_id}: {exc}")
            await asyncio.sleep(0.5)

    @reminder_loop.error
    async def reminder_loop_error(self, error):
        log.error(f"Reminder loop crashed: {error}", exc_info=error)
        await asyncio.sleep(60)  # brief cooldown before restart
