...
def fetch_cr_api(endpoint: str) -> dict | None:
    """Synchronous HTTP request for the Waitress/Flask threads."""
    headers = {
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card-box .card-level {
            ...
            background: #2a2a2a;
            color: {% if card.level == 16 %}#f1c40f{% else %}#aaa{% endif %};
            font-size: 0.75rem;
            border-radius: 4px;
            padding: 2px 7px;
        }
        .maxed { border-color: #f1c40f !important; }
    </style>
</head>
<body>
    ...
    <div class="header">
        ...
        {% if data.clan %}
            <div class="clan-badge">
                🛡️ <strong>{{ data.clan.name }}</strong>
    ...
    <div class="grid">
        <div class="stat-box">
            ...
            <div class="value blue">
                {% if data.leagueStatistics.currentSeason %}
                    Rating: {{ data.leagueStatistics.currentSeason.rank if data.leagueStatistics.currentSeason.rank else data.leagueStatistics.currentSeason.trophies }}
                {% else %}—{% endif %}
            </div>
        </div>
        <div class="stat-box">
            ...
            <div class="value white">
                {% if data.leagueStatistics.bestSeason %}
                    Rating: {{ data.leagueStatistics.bestSeason.rank if data.leagueStatistics.bestSeason.rank else data.leagueStatistics.bestSeason.trophies }}
                {% else %}—{% endif %}
            </div>
        </div>
    </div>
    ...
    <div class="grid">
        ...
        <div class="stat-box">
            <div class="label">Total Cards Found</div>
            <div class="value white">{{ counts.total }} / 121</div>
        </div>
        <div class="stat-box">
            <div class="label">Maxed (Lvl 16)</div>
            <div class="value green">{{ counts.lvl16 }}</div>
        </div>
        <div class="stat-box">
            <div class="label">Elite (Lvl 15)</div>
            <div class="value blue">{{ counts.lvl15 }}</div>
        </div>
        <div class="stat-box">
            <div class="label">Lvl 14</div>
            <div class="value white">{{ counts.lvl14 }}</div>
        </div>
    </div>
    ...
    <div class="grid">
        ...
        <div class="stat-box">
            ...
            <div class="value {% if data.battleCount > 0 and (data.wins / data.battleCount * 100) >= 50 %}green{% else %}red{% endif %}">
                {% if data.battleCount > 0 %}
                    {{ "%.1f" | format(data.wins / data.battleCount * 100) }}%
                {% else %}
                    —
                {% endif %}
            </div>
    ...
    <div class="grid">
        <div class="stat-box">
            ...
            <div class="value white">{{ data.totalDonations }}</div>
        </div>
        <div class="stat-box">
            <div class="label">War Day Wins</div>
            <div class="value white">{{ data.warDayWins }}</div>
        </div>
        <div class="stat-box">
            ...
            <div class="value blue">
                ...
                {% for badge in data.badges %}
                    {% if 'Played' in badge.name and 'Year' in badge.name %}
                        {% set age.found = true %}
                        {% set age.years = badge.progress %}
                    {% endif %}
                {% endfor %}
        ...
        {% for card in data.currentDeck %}
            <div class="card-box {% if card.level == 16 %}maxed{% endif %}">
                ...
                <span class="card-level"
                    style="color: {% if card.level == 16 %}#f1c40f{% else %}#aaa{% endif %}">
                    Lvl {{ card.level }}{% if card.level == 16 %} ✓{% endif %}
                </span>
            </div>
        {% endfor %}
    </div>
...
# ── Flask Routes (sync — required for Waitress/WSGI) ─────────────────────── #
...
def index():
    ...
    return render_template_string(ROSTER_HTML, members=members)
...
class GraveyardBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        if redis_url:
            ...
            log.info("ℹ️ No REDIS_URL provided. Using MongoDB for caching.")
...
if __name__ == "__main__":
    # Start Waitress in a background daemon thread so it doesn't block the bot.
